#![deny(missing_docs)]
//! Taken from https://raw.githubusercontent.com/mvlabat/bevy_egui and adapted for use with Rose.
...
pub struct EguiContext {
    ctx: HashMap<WindowId, ThreadGuard<rose_ui::Ui>>,
...
impl EguiContext {
    ...
    pub fn get_handler(&mut self, id: WindowId) -> Option<&mut rose_ui::Ui> {
        self.ctx.get_mut(&id).and_then(|g| g.get_mut())
    }

    pub fn set_cursor_position(&mut self, id: WindowId, position: egui::Vec2) {
        self.mouse_position.replace((id, position));
    }
}
